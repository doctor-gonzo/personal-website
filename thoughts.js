(function () {
  ...
  const featuredList = document.querySelector("[data-featured-tweet-list]");
  const featuredTweetId = featuredList?.dataset.featuredTweetId || "";
  ...
  const supportedThoughtModes = new Set(["featured", "tweets", "posts"]);
  const requestedThoughtMode = window.location.hash.slice(1).toLowerCase();
  const initialThoughtMode = supportedThoughtModes.has(requestedThoughtMode)
    ? requestedThoughtMode
    : "featured";
  selectThoughtMode(initialThoughtMode, { updateHash: false });

  function clearList(targetList = list) {
    while (targetList.firstChild) {
      targetList.removeChild(targetList.firstChild);
    }
  }

  function appendEmpty(message, targetList = list) {
  ...
  function appendTweet(tweet, targetList = list) {
    const item = document.createElement("li");
    item.className = "tweet-item";

    const authorRow = document.createElement("div");
    authorRow.className = "tweet-author-row";

    const profileLink = document.createElement("a");
    profileLink.href = "https://x.com/charlie______t";
    profileLink.className = "tweet-profile-link";
    profileLink.setAttribute("aria-label", "Open Charlie Thompson's Twitter profile");

    const profileImage = document.createElement("img");
    const profileImageUrl = "./assets/social/charlie-thompson-twitter.jpg";
    profileImage.src = window.__withSiteRelease
      ? window.__withSiteRelease(profileImageUrl)
      : profileImageUrl;
    profileImage.alt = "Charlie Thompson";
    profileImage.className = "tweet-profile-image";
    profileImage.width = 56;
    profileImage.height = 56;
    profileImage.loading = targetList === featuredList ? "eager" : "lazy";
    profileImage.decoding = "async";
    profileLink.appendChild(profileImage);
    authorRow.appendChild(profileLink);
  ...
  function renderFilteredTweets() {
    ...
    if (posts.length === 0) {
      ...
      posts.forEach((post) => appendTweet(post));
    }
  ...
  function renderFeaturedTweet() {
    if (!featuredList) return;

    const featuredTweet = postsById.get(String(featuredTweetId));
    clearList(featuredList);

    if (!featuredTweet) {
      appendEmpty("Featured tweet unavailable.", featuredList);
      return;
    }

    appendTweet(featuredTweet, featuredList);
  }
  ...
  const archiveUrl = window.__withSiteRelease
    ? window.__withSiteRelease(source)
    : source;
