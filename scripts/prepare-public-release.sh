...
RELEASE_ID="${SITE_RELEASE_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
node "$ROOT/scripts/stamp-public-release.mjs" "$DEST" "$RELEASE_ID"
...
echo "Prepared clean public snapshot: $DEST ($RELEASE_ID)"
